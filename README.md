...
1. **관리자 비밀번호** 설정 (선택, 비워두면 누구나 접근 가능)
2. **씨미 연동** 설정 (선택, 마법사에 준비 방법이 안내되어 있어!)

설정을 마치면 **자동으로 재시작**되고 대시보드로 이동해! 건너뛰면 테스트 모드로 바로 시작할 수 있어. 나중에 대시보드 설정 탭에서 언제든 변경 가능!

비밀번호를 설정했으면 **로그인 페이지**가 나와. 브라우저가 비밀번호를 기억해줘서 한 번만 입력하면 돼!

### 환경변수로 설정 바꾸기

대부분의 설정은 **대시보드 설정 탭**에서 바로 변경할 수 있어! 환경변수는 고급 사용자를 위한 거야.

`.env.example`을 `.env`로 복사해서 편집하면 돼. 전부 선택사항이야!
...
- [x] 첫 실행 설정 마법사 (가이드 포함, 씨미 개발자 센터 링크, 자동 재시작)
- [x] 대시보드 설정 탭에서 시스템/게임 설정 편집 (환경변수 편집 불필요!)
